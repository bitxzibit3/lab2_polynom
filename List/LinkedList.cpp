...
Node<T> *new_node(T new_value) {
    Node<T> *ans = (Node <T> *)calloc (1, sizeof(Node<T>));
    ans->set_value(new_value);
    ans->set_next_node(NULL);
    return ans;
}
...
template<class T>
void Node<T>::set_next_node(Node <T> *new_nod) {
    this->next = new_nod;
}
...
template<class T>
LinkedList<T>::LinkedList(T *items, size_t size1) {
