...
Polynom<T>::Polynom(ListSeq<T> *list) {
    this->data = (Sequence<T> *) new ListSeq<T>(list);
    this->seqtype = 1;
}

template<class T>
Polynom<T>::Polynom(ArrSeq<T> *seq) {
    this->data = (Sequence<T> *) new ArrSeq<T>(seq);
    this->seqtype = 2;
}
...
template<class T>
Polynom<T> *Polynom<T>::Copy() {
    if (this->seqtype == 1)
        return new Polynom<T> ((ListSeq<T> *)this->GetData()->Copy());
    else
        return new Polynom<T> ((ArrSeq<T> *)this->GetData()->Copy());
}

template<class T>
Polynom<T> *Polynom<T>::Derriativte()
{
    size_t checker = 0;
    Polynom<T> *ans = new_Polynom(this);
    if (this->Size() == 1)
    {
        ans->data->Prepend((T)0.);
        return ans;
    }
    ans->Resize(this->Size() - 1);
    for (size_t i = ans->Size() - 1; i > 0; i--)
    {
        ans->data->Set(i, this->Get(i + 1, &checker) * ((int)(i + 1)), &checker);
    }
    ans->data->Set(0, this->data->Get(1, &checker), &checker);
    return ans;
}
